...
bitflags! {
	/// Direction of an IOCTL call
	#[repr(transparent)]
	#[derive(Debug, Copy, Clone, Default, PartialEq, Eq)]
	pub struct IoCtlDirection: u8 {
		/// IOCTL Write: userland is writing / kernel is reading
		const IOC_WRITE = 1;

		/// IOCTL Read: userland is reading / kernel is writing
		const IOC_READ = 2;
	}
}
...
mod tests {
	...
	fn ioctl_call_correctly_written() {
		...
		let ioctl_call_number = (u32::from(call_size) << 16)
			| (u32::from(call_dir.bits()) << 30)
	...
	fn ioctl_call_correctly_parsed() {
		let ioctl_call_number = 0x8008_667e;

		let call_nr = 0x7eu8;
		let call_type = 0x66u8;
		let call_dir = IoCtlDirection::IOC_READ;
		let call_size = 8u16;
