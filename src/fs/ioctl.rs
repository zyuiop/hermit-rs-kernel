...
use crate::fd::{AccessPermission, Fd};
...
pub(crate) fn register_ioctl(path: &str, ioctl_object: Arc<async_lock::RwLock<Fd>>) {
	...
	if !directory.is_empty() {
		let _ = fs.root.traverse_mkdir(&directory, AccessPermission::all()); // ignore possible errors at this step
	}

	// Mount the file
	fs.root
		.traverse_mount(&path, Box::new(IoCtlNode(ioctl_object)))
