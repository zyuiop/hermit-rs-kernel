...
use crate::arch::kernel::amd_sev::opcodes::opcode;
...
impl IoIoExitInfo {
	...
	pub fn from_instruction(idata: &mut InstructionData, registers_data: &SavedRegisters) -> IoIoExitInfo {
		...
		match idata.operation() {
            KnownOpcode::IO_INS_BYTE | KnownOpcode::IO_INS_WORDS => {
				out.flags.insert(IoIoExitFlags::INPUT);
				out.flags.insert(IoIoExitFlags::STRING);
				out.port = (registers_data.rdx & 0xffff) as u16;
			}
			KnownOpcode::IO_OUTS_BYTE | KnownOpcode::IO_OUTS_WORDS => {
				out.flags.insert(IoIoExitFlags::STRING);
				out.segment_number = 0x3; // DS segment
				out.port = (registers_data.rdx & 0xffff) as u16;
			}
			KnownOpcode::IO_IN_BYTE_IMM | KnownOpcode::IO_IN_WORDS_IMM => {
				out.flags.insert(IoIoExitFlags::INPUT);

				// Read immediate value
				out.port = unsafe { idata.read_immediate(1)[0] as u16 };
			}
			KnownOpcode::IO_OUT_BYTE_IMM | KnownOpcode::IO_OUT_WORDS_IMM => {
				// Read immediate value
				out.port = unsafe { idata.read_immediate(1)[0] as u16 };
			}
			KnownOpcode::IO_IN_BYTE_DX | KnownOpcode::IO_IN_WORDS_DX => {
				out.flags.insert(IoIoExitFlags::INPUT);
				out.port = (registers_data.rdx & 0xffff) as u16;
			}
			KnownOpcode::IO_OUT_BYTE_DX | KnownOpcode::IO_OUT_WORDS_DX => {
				out.port = (registers_data.rdx & 0xffff) as u16;
			}
			other => {
		...
		out.flags.insert(match idata.operation() {
			KnownOpcode::IO_OUT_BYTE_IMM
			| KnownOpcode::IO_OUT_BYTE_DX
			| KnownOpcode::IO_OUTS_BYTE
			| KnownOpcode::IO_IN_BYTE_IMM
			| KnownOpcode::IO_IN_BYTE_DX
			| KnownOpcode::IO_INS_BYTE => IoIoExitFlags::DATA_8B,
			_ => {
