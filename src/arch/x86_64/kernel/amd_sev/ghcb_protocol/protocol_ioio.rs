...
use super::{GhcbExitCode, GhcbProtocolError, checked_vmgexit, error_exit_codes};
use super::allocated_ghcb::with_ghcb;
use super::ghcb::{Ghcb, GhcbU64Field};
use super::super::instruction_parser::{InstructionData, Size};
use super::super::opcodes::opcode::KnownOpcode;
use super::super::vc_handler::SavedRegisters;
...
impl<'a> IoIoExplicitProtocolExit<'a> {
	...
	fn write_to_ghcb(&self, ghcb: &mut Ghcb) {
		...
		ghcb.set_field(
			...
			match &self.data {
				...
				IoIoExplicitProtocolExitData::ByteIn(_)
				| IoIoExplicitProtocolExitData::WordIn(_)
				| IoIoExplicitProtocolExitData::DblWordIn(_) => 0,
				_ => sev_exit!(
					error_exit_codes::EXIT_OTHER,
					"not implemented: string operations are not implemented in the explicit IOIO handler"
				),
			},
		);
	}
	...
	fn read_response_from_ghcb(&mut self, ghcb: &Ghcb) {
		let Ok(rax) = ghcb.get_field_if_valid(GhcbU64Field::Rax) else {
			sev_exit!(
				error_exit_codes::EXIT_VC_ERROR,
				"invalid VMM response: rax is invalid"
			);
		};
		...
		match &mut self.data {
			...
			IoIoExplicitProtocolExitData::ByteOut(_)
			| IoIoExplicitProtocolExitData::WordOut(_)
			| IoIoExplicitProtocolExitData::DblWordOut(_) => { /* nothing to do */ }
			_ => sev_exit!(
				error_exit_codes::EXIT_OTHER,
				"not implemented: string operations are not implemented in the explicit IOIO handler"
			),
...
pub fn outb(port: u16, val: u8) {
	IoIoExplicitProtocolExit::new(port, IoIoExplicitProtocolExitData::ByteOut(val))
		.execute()
		.unwrap()
}
...
pub fn out_u32(port: u16, val: u32) {
	IoIoExplicitProtocolExit::new(port, IoIoExplicitProtocolExitData::DblWordOut(val))
		.execute()
		.unwrap()
}
...
pub fn in_u32(port: u16) -> u32 {
	let mut ret: u32 = 0;
	IoIoExplicitProtocolExit::new(port, IoIoExplicitProtocolExitData::DblWordIn(&mut ret))
		.execute()
		.unwrap();
	ret
}
...
pub fn inb(port: u16) -> u8 {
	let mut ret: u8 = 0;
	IoIoExplicitProtocolExit::new(port, IoIoExplicitProtocolExitData::ByteIn(&mut ret))
		.execute()
		.unwrap();
	ret
}
...
impl IoIoExitInfo {
	...
	pub fn from_instruction(
		...
		match idata.operation() {
			...
			other => {
				sev_exit!(
					error_exit_codes::EXIT_VC_INVALIDOP,
					"invalid ioio opcode {other:?}"
				);
			}
		};
