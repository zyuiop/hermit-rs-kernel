...
use crate::env::kernel::amd_sev::ghcb_protocol::ghcb::Ghcb;
use crate::env::kernel::amd_sev::ghcb_protocol::{checked_vmgexit, GhcbExitCode, PostProcessingError};
use crate::env::kernel::amd_sev::secrets::{get_key, get_next_available_key, get_sequence_number, increase_sequence_number, CommunicationKeyNumber};
use aes_gcm::aead::consts::{U12, U16};
use aes_gcm::aead::{Aead, AeadMut, Payload};
...
struct RequestPageHeader {
    ...
    /// The key number used for this message. See [`super::super::secrets::CommunicationKeyNumber`]
    pub vmkey: u8,
...
fn write_request_raw(req_page: &mut InterruptSpinMutexGuard<SNPSharedPage>, message_type: MessageType, mut request: Vec<u8>) {
    ...
    // Get the next key
    let (key_no, key, seqno) = get_next_available_key().expect("key space exhausted");
    increase_sequence_number(key_no);
    let seqno = seqno + 1;
