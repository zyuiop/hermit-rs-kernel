...
use super::{GhcbExitCode, GhcbProtocolError, checked_vmgexit};
use super::allocated_ghcb::with_ghcb;
use super::ghcb::{Ghcb, GhcbU64Field};
use super::super::vc_handler::InterruptStackFrame;
...
trait HypercallRaw {
	unsafe fn send_raw(
		...
		frame.registers.rax = unsafe {
			self.send_raw_values(
				ghcb,
				frame.registers.rbx,
				frame.registers.rcx,
				frame.registers.rdx,
...
impl HypercallNoArg {
	pub fn call(&self) -> Result<u64, GhcbProtocolError> {
		without_interrupts(|| with_ghcb(|ghcb| unsafe { self.send_raw_values(ghcb, 0, 0, 0, 0) }))
	}
}
...
impl Hypercall1Arg {
	pub fn call(&self, arg1: u64) -> Result<u64, GhcbProtocolError> {
		without_interrupts(|| {
			with_ghcb(|ghcb| unsafe { self.send_raw_values(ghcb, arg1, 0, 0, 0) })
		})
	}
}
...
impl Hypercall2Args {
	pub fn call(&self, arg1: u64, arg2: u64) -> Result<u64, GhcbProtocolError> {
		without_interrupts(|| {
			with_ghcb(|ghcb| unsafe { self.send_raw_values(ghcb, arg1, arg2, 0, 0) })
		})
	}
}
...
impl Hypercall3Args {
	pub fn call(&self, arg1: u64, arg2: u64, arg3: u64) -> Result<u64, GhcbProtocolError> {
		without_interrupts(|| {
			with_ghcb(|ghcb| unsafe { self.send_raw_values(ghcb, arg1, arg2, arg3, 0) })
		})
	}
}
...
impl Hypercall4Args {
	pub fn call(
		...
		without_interrupts(|| {
			with_ghcb(|ghcb| unsafe { self.send_raw_values(ghcb, arg1, arg2, arg3, arg4) })
		})
	}
}
