...
/// SAFETY: the source_addr must be valid
pub unsafe fn ghcb_mmio_read(source_addr: PhysAddr, output: &mut [u8]) -> Result<(), GhcbProtocolError> {
    without_interrupts(|| {
        with_ghcb(|ghcb| {
            unsafe  {
...
pub unsafe fn ghcb_mmio_write(input: &[u8], target_addr: PhysAddr) -> Result<(), GhcbProtocolError> {
    without_interrupts(|| {
        with_ghcb(|ghcb| {
            unsafe  {
