...
use super::ghcb::GhcbU64Field;
use super::protocol_page_state_change::PageStateChangePageSize::{PageSize2MB, PageSize4KB};
use super::GhcbProtocolError;
use super::{checked_vmgexit, GhcbExitCode};
...
pub fn change_page_states(changes: &[PageStateChangeEntry]) -> Result<(), ChangePageStateError> {
    ...
    let (_, exit2, chg_header) = without_interrupts(|| with_ghcb(|ghcb| {
        ghcb.clear();
        ...
        unsafe {
            let shared_buffer = ghcb.shared_buffer_raw_mut();
            let shared_buffer: *mut u64 = shared_buffer as *mut u64;
