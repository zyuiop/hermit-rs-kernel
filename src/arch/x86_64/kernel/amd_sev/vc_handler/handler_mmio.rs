...
impl VcHandler for MmioHandler {
    fn handle(&self, frame: &mut InterruptStackFrame, ghcb: &mut Ghcb, instruction_data: &mut InstructionData) -> Result<(), GhcbProtocolError> {
        ...
        match opcode {
            KnownOpcode::MOV_RM_REG_BYTE | KnownOpcode::MOV_RM_REG => {
            ...
            KnownOpcode::MOV_REG_RM_BYTE | KnownOpcode::MOV_REG_RM => {
                // MOV reg to reg/mem (read)
                let (register, address) = unsafe { instruction_data.parse_modrm_data(frame) };

                protocol_mmio::mmio_read(ghcb, map_address(address), register.as_mut_slice(frame, size))
            }
            KnownOpcode::MOVZ_REG_RM_BYTE | KnownOpcode::MOVZ_REG_RM => {
                // MOVZX regx, reg/memX
                // Read with zero extension
                let (register, address) = unsafe { instruction_data.parse_modrm_data(frame) };
                let size = if opcode == KnownOpcode::MOVZ_REG_RM_BYTE { 1 } else { 2 };
            ...
            KnownOpcode::MOV_RM_IMM_BYTE | KnownOpcode::MOV_RM_IMM => {
            ...
            KnownOpcode::OR_RM_IMM_BYTE | KnownOpcode::OR_RM_IMM => {
            ...
            KnownOpcode::TEST_RM_BYTE | KnownOpcode::TEST_RM => {
