/// This file is from the uart_16550 library.
/// It required being vendored to replace the io calls
...
impl SerialPort {
	...
	pub fn init(&mut self, _: Config) -> Result<(), ()> {
		// Disable interrupts
	...
	fn line_sts(&mut self) -> LineStsFlags {
		LineStsFlags::from_bits_truncate(inb(self.port_line_sts()))
	}
	...
	pub fn read(&mut self, out: &mut [u8]) -> Result<usize, Errno> {
		let mut index = 0;
		while index < out.len() {
			let Some(byte) = self.try_receive().ok() else {
				break;
			};
			out[index] = byte;
			index += 1;
