...
mod pcie {
	...
	impl McfgEntry {
		...
		fn is_base_address_valid(&self) -> bool {
			...
			let pci_end = self.pci_config_space_address(PCI_MAX_BUS_NUMBER, u8::MAX, u8::MAX).as_u64();
			...
			for mem_region in all_memory_regions {
					...
					|| (pci_start <= region_start && pci_end >= region_end) // PCI region contains the memory region
				{
					...
					error!("The declared PCI region {:x}-{:x} may overlap with physical memory region {:x}-{:x}",
						pci_start, pci_end, region_start, region_end
					);
					return false;
				}
			}
