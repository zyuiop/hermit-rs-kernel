...
impl TaskStacks {
	...
	pub fn from_boot_stacks() -> TaskStacks {
		...
		let mut ist_stacks = [const { MaybeUninit::<StackAllocation>::uninit() }; IST_ENTRIES];
		for i in 0..IST_ENTRIES {
			ist_stacks[i]  = MaybeUninit::new(core_local.interrupt_stack_allocs[i].borrow().as_ref().expect("no ist stack").weak());
		}
		let ist_stacks: MaybeUninit<[StackAllocation; 4]> = ist_stacks.into();
	...
	pub fn get_user_stack_size(&self) -> usize {
		match self.user_stack.as_ref() {
			None => 0,
			Some(stack) => stack.stack_size()
		}
	}
