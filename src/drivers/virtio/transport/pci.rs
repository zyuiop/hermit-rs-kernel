...
#[cfg(feature = "amd-sev")]
use crate::arch::kernel::amd_sev::ghcb_protocol::protocol_mmio::{mmio_read_volatile, mmio_write_volatile, ghcb_mmio_write};
...
impl NotifCtrl {
	...
	pub fn notify_dev(&self, data: NotificationData) {
		...
		if self.f_notif_data {
			unsafe {
				mmio_write_volatile(data.into_bits(), self.notif_addr).expect("failed to notify device");
			}
		} else {
			unsafe {
				mmio_write_volatile::<le16>(data.vqn().into(), self.notif_addr).expect("failed to notify device");
			}
		};
	}
}
...
impl IsrStatus {
	...
	fn new(mut raw: VolatileRef<'static, IsrStatusRaw>) -> Self {
		...
		IsrStatus { interrupt_status: DeviceAlloc.phys_addr_from(ptr) }
	}
	...
	pub fn acknowledge(&mut self) -> IsrStatusRaw {
		unsafe {
			mmio_read_volatile(self.interrupt_status)
				.expect("failed to read isr status")
		}
	}
}
